...
public final class ActionBattleDamageHudRenderer {
    ...
    public static void renderTrailingHp(GuiGraphics graphics, ActionBattleHudLayout.Rect rect, boolean flipped,
                                        int currentHp, int maxHp, double trailingHp, int obscurityStage) {
        ...
        int baseX = infoX - 2;
        if (!flipped) {
            graphics.fill(baseX + currentWidth, rect.y() + 22, baseX + trailingWidth, rect.y() + 26,
                    ActionBattleObscurityHudRules.presentationColor(LOST_HP, obscurityStage));
        } else {
            int trailingLeft = baseX + (HP_BAR_WIDTH - trailingWidth);
            int currentLeft = baseX + (HP_BAR_WIDTH - currentWidth);
            graphics.fill(trailingLeft, rect.y() + 22, currentLeft, rect.y() + 26,
                    ActionBattleObscurityHudRules.presentationColor(LOST_HP, obscurityStage));
        }
    }
    public static void renderFloating(GuiGraphics graphics, Font font, ActionBattleHudLayout.Rect rect, boolean ally,
                                      ActionBattleDamageHudState.RenderSnapshot snapshot, int obscurityStage) {
        if (snapshot == null || snapshot.floatingDamage().isEmpty()
                || !ActionBattleObscurityHudRules.showsFloatingDamage(obscurityStage)) return;
        for (ActionBattleDamageHudState.FloatingDamage event : snapshot.floatingDamage()) {
            ...
            int color = ActionBattleObscurityHudRules.presentationColor((alpha << 24) | rgb, obscurityStage);
            Component text = obscuredText("-" + event.damage(), obscurityStage);
            int anchorX = ally ? rect.x() - 4 - outward : rect.x() + rect.width() + 4 + outward;
            drawScaled(graphics, font, text, anchorX, y, 0.70F, color, ally);
        }
    }
    private static void drawScaled(GuiGraphics graphics, Font font, Component text, int anchorX, int y, float scale, int color, boolean rightAligned) {
    ...
    private static Component obscuredText(String text, int stage) {
        Component result = Component.literal(text);
